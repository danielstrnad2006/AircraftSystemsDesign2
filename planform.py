...
class WingSizing:
    def __init__(self, S_w, b, c_root, c_tip, taper_ratio,
        ...
        self.trailing_sweep = math.degrees(
                math.atan(math.tan(math.radians(self.quart_sweep)) +
                          3 * (self.c_root / (2 * self.b)) * (self.taper_ratio - 1))
            )
    ...
    def plot(self, chord_position=0):
        base_pickle_path = 'temp/base_planform.pickle'

        # Ensure temp folder exists
        if not os.path.exists('temp'):
            os.mkdir('temp')

        if os.path.exists(base_pickle_path):
            # Load pickled base figure
            with open(base_pickle_path, 'rb') as f:
                fig, ax = pickle.load(f)
            ax = fig.axes[0]  # use the first Axes object
        else:
            # Create new figure for the base planform
            ...
            # Fill area first
            ax.fill_between(y, -LE, -TE, color='gray', alpha=0.3)
            # Plot lines on top
            ax.plot(y, -LE, label='Leading Edge', color='gray', linewidth=2)
            ax.plot(y, -TE, label='Trailing Edge', color='gray', linewidth=2)

            # Fixed axes
            ax.set_xlim(0, max(y) * 1.1)
            ax.set_ylim(-max(TE) * 1.1, 0)
            ax.set_xlabel("y [m]")
            ax.set_ylabel("x [m]")

            # Pickle the base planform for reuse
            with open(base_pickle_path, 'wb') as f:
                pickle.dump((fig, ax), f)
        # Add red vertical line at chord position
        ax.plot([chord_position] * 2, [-self.leading_edge(chord_position), -self.trailing_edge(chord_position)],
